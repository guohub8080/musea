/**
 * TonicMLDocument 用户指引动态加载器
 * 自动扫描 src/books/TonicMLDocument 文件夹下的所有 .tsx, .md, .mdx 文件
 * 按文件夹分组，每个文件夹对应一个分类
 * 
 * 文件夹命名规则：[x]M
 * - [x] - 排序号，数字越小越靠上
...
import { tonicmlDocumentConfig } from './info.tsx';
...
function groupAndSortDocuments(): DocumentCategory[] {
  ...
  Object.entries(documentModules).forEach(([path, module]) => {
    ...
    // 检查是否有重复的 URL
    const fullUrl = `${tonicmlDocumentConfig.slug}/${routePath}`;
...
// 创建 BookLoader 实现
const tonicmlDocumentLoader: BookLoader = {
  config: {
    title: tonicmlDocumentConfig.title,
    slug: tonicmlDocumentConfig.slug,
    icon: () => tonicmlDocumentConfig.icon,
    description: tonicmlDocumentConfig.description
  },
...
// 生成路由的函数
export function generateTonicMLDocumentRoutes(): RouteObject[] {
  return [
    {
      path: tonicmlDocumentConfig.slug,
      element: (
        <BookLayoutConfigProvider basePrefix={`/${tonicmlDocumentConfig.slug}`}>
          <BookLayout loader={tonicmlDocumentLoader} />
      ...
      children: [
        ...
        ...userDocumentCategories.flatMap((category) =>
          category.articles.map((article) => {
            ...
            return {
              path: path,
              element: <ArticleContent loader={tonicmlDocumentLoader}>{article.content}</ArticleContent>,
            };
          })
        ),
      ],
...
export default tonicmlDocumentLoader;
