/**
 * TonicMLScore 用户指引动态加载器
 * 自动扫描 src/books/TonicMLScore 文件夹下的所有 .tsx, .md, .mdx 文件
 * 按文件夹分组，每个文件夹对应一个分类
 * 
 * 文件夹命名规则：[x]M
 * - [x] - 排序号，数字越小越靠上
...
import { tonicmlScoreConfig } from './info.tsx';
...
function groupAndSortDocuments(): DocumentCategory[] {
  ...
  Object.entries(documentModules).forEach(([path, module]) => {
    ...
    // 检查是否有重复的 URL
    const fullUrl = `${tonicmlScoreConfig.slug}/${routePath}`;
...
// 创建 BookLoader 实现
const tonicmlScoreLoader: BookLoader = {
  config: {
    title: tonicmlScoreConfig.title,
    slug: tonicmlScoreConfig.slug,
    icon: () => tonicmlScoreConfig.icon,
    description: tonicmlScoreConfig.description
  },
...
// 生成路由的函数
export function generateTonicMLScoreRoutes(): RouteObject[] {
  return [
    {
      path: tonicmlScoreConfig.slug,
      element: (
        <BookLayoutConfigProvider basePrefix={`/${tonicmlScoreConfig.slug}`}>
          <BookLayout loader={tonicmlScoreLoader} />
      ...
      children: [
        ...
        ...userDocumentCategories.flatMap((category) =>
          category.articles.map((article) => {
            ...
            return {
              path: path,
              element: <ArticleContent loader={tonicmlScoreLoader}>{article.content}</ArticleContent>,
            };
          })
        ),
      ],
...
export default tonicmlScoreLoader;
